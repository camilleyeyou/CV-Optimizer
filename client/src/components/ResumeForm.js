...
const ResumeForm = () => {
  ...
  const handleUpdateExperience = (index, field, value) => {
    if (index < 0 || index >= workExperience.length) {
      console.error(`Invalid experience index: ${index}`);
      return;
    }
    const updatedExperience = { ...workExperience[index], [field]: value };
    updateListItem('workExperience', index, updatedExperience);
  };

  const handleAddHighlight = (experienceIndex) => {
    if (experienceIndex < 0 || experienceIndex >= workExperience.length) {
      console.error(`Invalid experience index: ${experienceIndex}`);
      return;
    }
    const experience = workExperience[experienceIndex];
  ...
  const handleUpdateHighlight = (experienceIndex, highlightIndex, value) => {
    ...
    if (highlightIndex < 0 || highlightIndex >= highlights.length) {
      console.error(`Invalid highlight index: ${highlightIndex}`);
      return;
    }
